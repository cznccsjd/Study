 ...
 *  @author Kevin Wayne
 */
public final class StdRandomLocal {
    ...
    public static void main(String[] args) {
        ...
        if (args.length == 2) StdRandomLocal.setSeed(Long.parseLong(args[1]));
        double[] probabilities = { 0.5, 0.3, 0.1, 0.1 };
        int[] frequencies = { 5, 3, 1, 1 };
        String[] a = "A B C D E F G".split(" ");

        StdOutLocal.println("seed = " + StdRandomLocal.getSeed());
        for (int i = 0; i < n; i++) {
            StdOutLocal.printf("%2d ",   uniform(100));
            StdOutLocal.printf("%8.5f ", uniform(10.0, 99.0));
            StdOutLocal.printf("%5b ",   bernoulli(0.5));
            StdOutLocal.printf("%7.5f ", gaussian(9.0, 0.2));
            StdOutLocal.printf("%1d ",   discrete(probabilities));
            StdOutLocal.printf("%1d ",   discrete(frequencies));
            StdOutLocal.printf("%11d ",  uniform(100000000000L));
            StdRandomLocal.shuffle(a);
            for (String s : a)
                StdOutLocal.print(s);
            StdOutLocal.println();
        }
