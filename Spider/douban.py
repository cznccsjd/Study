...
class Douban():
    ...
    def search(self):
        ...
        print('开始运行组url循环：',time.strftime("%H:%M:%S",time.localtime()))


########################################
        for num in range (0, 40, 20):      #通过豆瓣页面查询到的，当start>200时，会跳转到登录页面了
            url, data = Douban().searchUrl(num)
            try:

                print('组url单次请求开始：',time.strftime('%H:%M:%S',time.localtime()))
                ...
                print('组url单次请求结束：',time.strftime('%H:%M:%S',time.localtime()))
            ...
            print('组beautifulsoup单次处理开始:',time.strftime('%H:%M:%S',time.localtime()))
            ...
            for title, link in  zip(titles, links):
                ...
                if ret:
                    ...
                    print('组beautifulsoup单次处理结束：',time.strftime('%H:%M:%S',time.localtime()))
